...
{
	var win = window.open( 'https://web.archive.org/web/20140131214109/http://store.steampowered.com/gotflash','gotflash','width=536,height=546,resize=yes,scrollbars=yes');
	win.focus();
}
...
function PageTab( tab, delta, max, params )
{
	...
	if ( tabStart[tab] > tabMax[tab] )
	{
		...
		new Ajax.Updater( 
				'tab_' + tab + '_items', 
				'https://web.archive.org/web/20140131214109/http://store.steampowered.com/search/tab', 
...
{	
	...
	if ( elem.curPos > elem.maxLoaded )
	{
		...
		new Ajax.Updater( 
				targetid, 
				'https://web.archive.org/web/20140131214109/http://store.steampowered.com/search/smallcapscroll', 
			...
			window.setTimeout( function() { 
				if ( elem.bWantsHover && !elem.ajaxRequest ) {
					elem.ajaxRequest = new Ajax.Updater( hover.down('.content'),
								'https://web.archive.org/web/20140131214109/http://store.steampowered.com/' + strUrlTarget + rgHoverData['id'],
...
{
	var url = 'https://web.archive.org/web/20140131214109/http://store.steampowered.com/api/addtowishlist';
	if ( navref )
...
{
	$(divBtn).hide();
	var url = 'https://web.archive.org/web/20140131214109/http://store.steampowered.com/friends/recommendgame';
	if ( navref )
		MakeNavCookie( navref, url );
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'https://web.archive.org/web/20140131214109/http://store.steampowered.com/recommended/ignorerecommendation/', {
		method: 'post',
...
function LaunchWebChat()
{
	...
	iframe.hide();
	iframe.src='https://web.archive.org/web/20140131214109/http://steamcommunity.com/chat/launch/'
...
function SearchTimeout( elem, value, elemSuggestionCtn, elemSuggestions )
{
	if ( value )
	{
		...
		new Ajax.Updater( elemSuggestions, 'https://web.archive.org/web/20140131214109/http://store.steampowered.com/search/suggest',
			{
				parameters: parameters,
				method: 'GET',
				onComplete: function() {
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		var url = 'https://web.archive.org/web/20140131214109/http://store.steampowered.com/tagdata/myfrequenttags';
		...
		$J.get( url ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'https://web.archive.org/web/20140131214109/http://store.steampowered.com/tag/en/' + encodeURIComponent( tag.name ) })
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
/*
     FILE ARCHIVED ON 21:41:09 Jan 31, 2014 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 16:53:58 Nov 26, 2022.
...
/*
playback timings (ms):
  captures_list: 187.656
  exclusion.robots: 0.254
  exclusion.robots.policy: 0.245
  RedisCDXSource: 0.649
  esindex: 0.01
  LoadShardBlock: 163.465 (3)
  PetaboxLoader3.datanode: 189.027 (5)
  CDXLines.iter: 19.829 (3)
  load_resource: 162.806 (2)
  PetaboxLoader3.resolve: 84.584 (2)
*/